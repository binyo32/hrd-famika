...
const AdminSettings = () => {
  ...
  const fetchData = useCallback(async (tableName, setData, loadingKey) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: `Gagal memuat data ${loadingKey}: ${error.message}`,
        variant: "destructive",
      });
    } finally {
  ...
  const handleFormSubmit = async (formData) => {
    ...
    try {
      ...
      toast({
        title: "Sukses",
        description: `${itemType} berhasil ${formData.id ? "diperbarui" : "ditambahkan"}.`,
      });
      ...
      toast({
        title: "Error",
        description: `Gagal menyimpan ${itemType.toLowerCase()}: ${error.message}`,
        variant: "destructive",
      });
    } finally {
  ...
  const handleDelete = async (id, type) => {
    const itemName = (type === "Divisi" ? divisions : jobPositions).find(
      (i) => i.id === id,
    )?.name;
    if (
      !window.confirm(
        `Apakah Anda yakin ingin menghapus ${type.toLowerCase()} "${itemName}"?`,
      )
    )
      return;
    ...
    try {
      ...
      toast({
        title: "Error",
        description: `Gagal menghapus ${type.toLowerCase()}: ${error.message}`,
        variant: "destructive",
      });
    }
  };
      ...
      <div className="space-y-8">
        <motion.div
          ...
          <p className="text-muted-foreground">
            Kelola data master seperti Divisi dan Jabatan untuk digunakan di
            seluruh sistem.
          </p>
        </motion.div>
